...
RUN wget http://ftp.riken.jp/net/apache/hadoop/common/hadoop-3.2.0/hadoop-3.2.0.tar.gz
RUN mv hadoop-3.2.0.tar.gz /tmp/
RUN tar -xzvf /tmp/hadoop-3.2.0.tar.gz -C /usr/
ENV HADOOP_HOME /usr/hadoop-3.2.0
...
RUN wget http://ftp.riken.jp/net/apache/hive/hive-3.1.1/apache-hive-3.1.1-bin.tar.gz
RUN mv apache-hive-3.1.1-bin.tar.gz /tmp/
RUN tar -xzvf /tmp/apache-hive-3.1.1-bin.tar.gz -C /tmp/
RUN mv /tmp/apache-hive-3.1.1-bin /usr/hive-3.1.1
RUN cp /usr/hive-3.1.1/conf/hive-default.xml.template /usr/hive-3.1.1/conf/hive-site.xml
ENV HIVE_HOME /usr/hive-3.1.1
...
RUN sed -i -e 's/<\/configuration>//' /usr/hive-3.1.1/conf/hive-site.xml
COPY hive-template /tmp/hive-template
RUN cat /tmp/hive-template >> /usr/hive-3.1.1/conf/hive-site.xml
